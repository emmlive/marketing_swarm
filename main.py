...
gemini_llm = LLM(
    ...
    temperature=0.4 # Grounded for anti-hallucination
)
...
def get_swarm_agents(inputs):
    ...
    return {
        "analyst": Agent(
            ...
            backstory="You are a literalist data scientist. You only report facts found via tools. If data is not found, state 'Data not found'.",
        ...
        "creative": Agent(
            ...
            goal="Transform Analyst data into Navy/White branded assets and cinematic video prompts for the Veo model.",
            backstory="You use the specific gaps identified by the Analyst to build high-conversion copy. You also specialize in writing vivid 'Video Prompts' for AI video generators.",
        ...
        "web_auditor": Agent(
            ...
            backstory="You use the Scrape tool to find 'Friction Points'. No guessing. You focus on CTA clarity and accessibility.",
        ...
        "geo_specialist": Agent(
            ...
            backstory="AI Search expert. You identify local keyword clusters to make the brand an LLM-cited authority.",
            llm=gemini_llm, verbose=True
        ),
        "strategist": Agent(
            role="Lead Strategist (The Swarm Commander)",
            goal="Orchestrate agents and validate outputs for ROI alignment.",
            backstory="Final gatekeeper. You ensure 100% brand consistency and fact-anchored ROI roadmap.",
            llm=gemini_llm, verbose=True
        )
    }
...
class MarketingSwarmFlow(Flow[SwarmState]):
    ...
    def phase_2_execution(self):
        ...
        creative_task = Task(
            description=(
                ...
                "The prompt should describe a 10-second cinematic corporate scene related to the business."
            ),
            agent=self.agents["creative"],
            expected_output="Grounded Branded Assets and a specific 'Video Prompt:' section."
        )
    ...
    def phase_3_validation(self):
        ...
        val_task = Task(
            description="Synthesize all grounded outputs into a 30-day brief. Ensure the roadmap is ROI-focused.",
