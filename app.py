...
# --- AGENT SEAT EXECUTION LOOP (TABS 1-9) ---
# We render these first to ensure the loop doesn't bleed into Tab 11/12
seats = [
...
# --- TAB 11: TEAM INTEL & HEATMAP (RESTORED FEATURES) ---
with tabs[11]:
    ...
    if not leads_df.empty:
        ...
        c1, c2 = st.columns(2)
        with c1:
            st.write("**📊 Industry Mix**")
            st.bar_chart(leads_df['industry'].value_counts())
        with c2:
            st.write("**📈 Status Pipeline**")
            st.line_chart(leads_df['status'].value_counts())
    else:
        st.info("No market data available yet.")
    conn.close()

# --- TAB 12: ⚡ GOD-MODE ADMIN CONTROL (ISOLATED & FIXED) ---
with tabs[12]:
    ...
    h1, h2 = st.columns(2)
    h1.metric("Total Database Records", len(leads_all))
    h2.metric("System Health", "Operational", delta="Stable")
    
    # 2. Data Governance Hub
    st.divider()
    st.subheader("🧹 System Maintenance Hub")
    
    admin_col1, admin_col2 = st.columns(2)
    with admin_col1:
        with st.expander("📥 Master Data Export"):
            csv = leads_all.to_csv(index=False).encode('utf-8')
            st.download_button("📥 Export All Leads (CSV)", csv, "master_leads.csv", "text/csv", use_container_width=True)
            st.caption("Download full database for CRM integration.")
    ...
    with admin_col2:
        with st.expander("🚨 Critical Reset Zone"):
            if st.button("Purge Demo Data", type="secondary", use_container_width=True):
                ...
                count = cursor.rowcount
                conn.commit()
                st.success(f"Successfully purged {count} demo records.")
                st.rerun()
